// Update License Status API (Combined: Approve and Reject)
// Use action: 'approve' or 'reject' in request body
...
export default async function handler(req, res) {
  ...
  const { licenseKey, action, expiryDate } = req.body;

  if (!licenseKey || !action) {
    return res.status(400).json({
      error: 'Validation error',
      message: 'License key and action (approve/reject) are required'
    });
  }
  ...
  if (action === 'approve' && !expiryDate) {
    return res.status(400).json({
      error: 'Validation error',
      message: 'Expiry date is required for approval'
    });
  }
